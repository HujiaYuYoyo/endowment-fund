...
Equations
	x1c1		'constraint 1 on x1'
	x1c2		'constraint 2 on x1'
	x1c3		'constraint 3 on x1'
	x1c4		'constraint 4 on x1'
	x1c5		'constraint 5 on x1'
...
x1c1..		x1('cash') =e= 0.0;
x1c2..		x1('stock') =e= 0.266;
x1c3..		x1('alter') =e= 0.726;
x1c4..		x1('gbond') =e= 0.0;
x1c5.. 		x1('cbond') =e= 0.0;
...
expvar.. 		sum((w1,w2,w3), wealth(w1,w2,w3))/nall =g= 0;
...
Model endowmentcvar /x1c1, x1c2, x1c3, x1c4, x1c5, vardefcon, objdefcvar, abalance1, cbalance1, arebalance2, expvar, termw2, crebalance3, arebalance3, crebalance2/;
...
File outofsampleresults / outofsampleresults.txt /;
put outofsampleresults;
put cvar.l, var.l /;
put "Stage 1"/;
 loop(i,
 put x1.l(i)/
 );
 put 'Total return at each scenario for stage 1'/;
* loop(w1,
* 	put sum(i, r1(i,w1)*x1.l(i)) /;
* );
 put 'Expected return at each scenario for stage 1'/;
 put sum(w1, sum(i, r1(i,w1)*x1.l(i)))/n1 /;

 put "Stage 2"/;
* loop((w2,w1),
* 	put 'Total return at each scenario for stage 2'/;
* 	put sum(i, r2(i,w2)*x2.l(i, w1))/;
* );

 put 'Expected return at each scenario for stage 2'/;
 put sum((w1,w2), sum(i, r2(i,w2)*x2.l(i, w1)))/(n1*n2)/;

 put "Stage 3"/;
* loop((w1, w2, w3),
* 	put 'Total return at each scenario for stage 3'/;
* 	put sum(i, r3(i,w3)*x3.l(i, w1, w2))/;
*);
 put 'Expected return at each scenario for stage 3'/;
 put sum((w1, w2, w3), sum(i, r3(i,w3)*x3.l(i, w1, w2)))/nall/;
 putclose;